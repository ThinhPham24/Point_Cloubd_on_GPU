...
import cv2
import time
from cvfun import show, read_stereo_paremater
...
def process_arguments():
    ...
    parser.add_argument('--backend', choices=['cuda','ofa','ofa-pva-vic'],default = 'cuda',
                        help='Backend to be used for processing')
    parser.add_argument('--left', default= 'SIZE_50/L_0_50.png',help='Rectified left input image from a stereo pair')
    parser.add_argument('--right',default= 'SIZE_50/R_0_50.png', help='Rectified right input image from a stereo pair')
    ...
    parser.add_argument('--window_size', default=7, type=int, help='Median filter window size')
    ...
    parser.add_argument('-p1', default=100, type=int, help='Penalty P1 on small disparities') #3
    parser.add_argument('-p2', default=250, type=int, help='Penalty P2 on large disparities')
    parser.add_argument('--p2_alpha', default=8, type=int, help='Alpha for adaptive P2 Penalty')
    parser.add_argument('--uniqueness', default=12, type=float, help='Uniqueness ratio')
    parser.add_argument('--skip_diagonal', default=False, action='store_true', help='Do not use diagonal paths')
    parser.add_argument('--num_passes', default=1, type=int, help='Number of passes')
    parser.add_argument('--min_disparity', default= 128, type=int, help='Minimum disparity')
...
def main():
    ...
    print("Reading image time:", time.time() - t1)
    t2 = time.time()
    # Preprocess input
    ...
    mask_L = cv2.imread('/home/airlab/Desktop/Thinh/mask_L_25.png', 0)
    disparity = cv2.bitwise_and(disparity, disparity, mask = mask_L)
    # depth = np.where(mask_L > 1e-5, disparity, 0)
  
    left_image_color = cv2.imread(args.left)
    ...
    # ply_path  = '/home/airlab/Desktop/Thinh/pc.ply'
    # write(ply_path, pcPoints,pcColors)
    # print("Image processing time:", time.time()-t2)
    # t3 = time.time()
